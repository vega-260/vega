...
export function PipelineBoard() {
  ...
  const currentApplicants = useMemo(() => {
    ...
    if (minScore > 0) {
      list = list.filter((a) => a.talent_score !== null && a.talent_score !== undefined && Number(a.talent_score) >= minScore);
    }
  ...
  const insights = useMemo(() => {
    ...
    const expert = currentApplicants.filter(
      (a) => a.talent_score !== null && a.talent_score !== undefined && Number(a.talent_score) > 80,
    ).length;
  ...
  const filteredApplicants = useMemo(() => {
    ...
    if (filterMatchScore !== "ALL") {
      if (filterMatchScore === "HIGH") {
        list = list.filter((a) => a.talent_score !== null && a.talent_score !== undefined && Number(a.talent_score) >= 85);
      } else if (filterMatchScore === "MID") {
        list = list.filter(
          (a) => a.talent_score !== null && a.talent_score !== undefined && Number(a.talent_score) >= 70 && Number(a.talent_score) < 85,
        );
      } else if (filterMatchScore === "LOW") {
        list = list.filter((a) => a.talent_score !== null && a.talent_score !== undefined && Number(a.talent_score) < 70);
      }
    }
  ...
  const sortedApplicants = useMemo(() => {
    ...
    switch (sortBy) {
      ...
      case "highest_match":
        list.sort((a, b) => {
          const scoreA = (a.talent_score !== null && a.talent_score !== undefined && !isNaN(Number(a.talent_score))) ? Number(a.talent_score) : null;
          const scoreB = (b.talent_score !== null && b.talent_score !== undefined && !isNaN(Number(b.talent_score))) ? Number(b.talent_score) : null;
  ...
  const handleExportCSV = (data: any[]) => {
    ...
    const rows = data.map((cand) => {
      ...
      return [
        ...
        cand.job_title || "—",
        cand.talent_score !== null && cand.talent_score !== undefined && !isNaN(Number(cand.talent_score)) ? `${Math.round(Number(cand.talent_score))}%` : 'Not available',
        skillsList.join(", "),
          ...
          >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-7 gap-2.5 xl:gap-3 px-1 pb-6">
              {PIPELINE_STAGES.map((stage) => {
                ...
                const scoredApps = stageApps.filter((a) => a.talent_score !== null && a.talent_score !== undefined && !isNaN(Number(a.talent_score)));
                const totalScore = scoredApps.reduce(
                  (acc, curr) => acc + Number(curr.talent_score),
                  0,
                );
                const avgScore =
                  scoredApps.length > 0
                    ? Math.round(totalScore / scoredApps.length)
                    : null;
                              ...
                              <div className="flex items-center gap-3">
                                ...
                                <div className="min-w-0">
                                  <div className="flex items-center gap-1">
                                    ...
                                    {cand.talent_score !== null && cand.talent_score !== undefined && !isNaN(Number(cand.talent_score)) && Number(cand.talent_score) >= 85 && (
                                      <Star
                                        size={11}
                              ...
                              <span
                                className={`px-2 py-1 text-[11px] font-black rounded-full border shadow-sm ${
                                  cand.talent_score !== null && cand.talent_score !== undefined && !isNaN(Number(cand.talent_score)) && Number(cand.talent_score) >= 85
                                    ...
                                    : cand.talent_score !== null && cand.talent_score !== undefined && !isNaN(Number(cand.talent_score)) && Number(cand.talent_score) >= 70
...
function CandidateCard({
  ...
  const hasScore = candidate.talent_score !== null && candidate.talent_score !== undefined && !isNaN(Number(candidate.talent_score));
  const matchScore = hasScore ? Math.round(Number(candidate.talent_score)) : null;
  ...
  if (matchScore !== null) {
    if (matchScore >= 85)
      ...
      recBadge = (
        <span className="bg-slate-100 border border-slate-200 text-slate-600 px-2 py-0.5 rounded-md text-[8px] font-semibold uppercase truncate shadow-sm">
          Core ({matchScore}%)
        </span>
      );
  }
        ...
        <div className="pb-4 border-b border-slate-100 flex justify-between items-start">
          ...
          <div className="bg-emerald-50 border border-emerald-100 rounded-xl p-1.5 px-2.5 text-center shadow-sm shrink-0">
            ...
            <div className="text-lg font-black text-emerald-700 leading-none">
              {candidate.talent_score !== null && candidate.talent_score !== undefined && !isNaN(Number(candidate.talent_score)) ? `${Math.round(Number(candidate.talent_score))}%` : 'Not available'}
            </div>
          </div>
        </div>
      </div>
